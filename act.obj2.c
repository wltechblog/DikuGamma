...
void do_drink(struct char_data *ch, char *argument, int cmd) {
  ...
  if ((GET_COND(ch, DRUNK) > 10) &&
      (GET_COND(ch, THIRST) > 0)) /* The pig is drunk */
  {
...
void do_eat(struct char_data *ch, char *argument, int cmd) {
  ...
  if (GET_COND(ch, FULL) > 20) act("You are full.", FALSE, ch, 0, 0, TO_CHAR);

  if (temp->obj_flags.value[3] &&
      (GET_LEVEL(ch) < 21)) /* The shit was poisoned ! */
  {
    act("Ooups, it tasted rather strange ?!!?", FALSE, ch, 0, 0, TO_CHAR);
...
void do_pour(struct char_data *ch, char *argument, int cmd) {
  char arg1[MAX_STRING_LENGTH - 32];
  char arg2[MAX_STRING_LENGTH - 32];
  ...
  switch (keyword) {
    case 0: { /* LIGHT SOURCE */
      if (ch->equipment[WEAR_LIGHT])
        ...
        if (obj_object->obj_flags.value[2]) world[ch->in_room].light++;
      }
    } break;
...
void do_wear(struct char_data *ch, char *argument, int cmd) {
  ...
  int keyword;
  static char *keywords[] = {"finger", "neck",   "body", "head",  "legs",
                             "feet",   "hands",  "arms", "about", "waist",
                             "wrist",  "shield", "\n"};
...
/* case 12: ...
                                        ...
                                        if ( ( (ch->player.class ==
   CLASS_CLERIC) && ( (GET_ITEM_TYPE(obj_object) == ITEM_WEAPON) &&
                                                (sharp[obj_object->obj_flags.value[3]])
   ) || ( (ch->player.class == CLASS_CLERIC) && ( (GET_ITEM_TYPE(obj_object) ==
   ITEM_FIREWEAPON) && (sharp[obj_object->obj_flags.value[3]]) ) ) ) ) { if
   (GET_ITEM_TYPE(obj_object) == ITEM_WEAPON) { strcpy(buffer, "You can't wield
   that! its SHARP.\n\r"); strcat(buffer, "You are forbidden to use sharp
   weapons!\n\r"); send_to_char(buffer, ch);
                                                        }
                                                        if
   (GET_ITEM_TYPE(obj_object) == ITEM_FIREWEAPON) { strcpy(buffer, "You can't
   wield that! it fires only SHARP things.\n\r"); strcat(buffer, "You are
   forbidden to use sharp weapons!\n\r"); send_to_char(buffer, ch);
                                                        }
                                                } else {
*/
...
/* case 13: ...
   ...
   && ( (GET_ITEM_TYPE(obj_object) == ITEM_MISSILE) &&
                                                (sharp[obj_object->obj_flags.value[3]])
   ) ) { strcpy(buffer, "This is a SHARP thing, to be used in a weapon.\n\r.");
                                                strcat(buffer, "You are
   forbidden to use sharp weapons!\n\r"); send_to_char(buffer, ch); } else {
*/
