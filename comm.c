...
int autoreboot = 0;        /* reboot the game after a shutdown */
...
int game_loop(int s) {
  ...
  while (!diku_shutdown) {
    ...
    if (FD_ISSET(s, &input_set))
      if (new_descriptor(s) < 0) perror("New connection");
    ...
    for (point = descriptor_list; point; point = next_point) {
      ...
      if (FD_ISSET(point->descriptor, &input_set))
        if (process_input(point) < 0) close_socket(point);
    }
    ...
    if (!(pulse % (SECS_PER_MUD_HOUR * 4))) {
      ...
      affect_update();
      point_update();
      if (time_info.hours == 1) update_time();
    }

    if (pulse >= 2400) {
      pulse = 0;
      if (lawful) night_watchman();
      check_reboot();
    }
...
int process_output(struct descriptor_data *t) {
  ...
  while (get_from_q(&t->output, i)) {
    ...
    if (write_to_descriptor(t->descriptor, i)) return (-1);
  }
    ...
    if (!ISNEWL(*(t->buf + i)) && !(flag = (k >= (MAX_INPUT_LENGTH - 2))))
      ...
      *(tmp + k) = 0;
      ...
      if (flag) {
        ...
        if (write_to_descriptor(t->descriptor, buffer) < 0) return (-1);
      ...
      for (squelch = 0;; squelch++)
        if ((*(t->buf + squelch) = *(t->buf + i + squelch)) == '\0') break;
      k = 0;
      i = 0;
    }
  }
...
void close_sockets(int s) {
  ...
  while (descriptor_list) close_socket(descriptor_list);
...
#define COMA_SIGN                                                          \
  "\n\rDikuMUD is currently inactive due to excessive load on the host "   \
  "machine.\n\rPlease try again later.\n\r\n\n\r  Sadly,\n\r\\n\r    the " \
  "DikuMUD system operators\n\r\n\r"
...
void send_to_outdoor(char *messg) {
  ...
  if (messg)
    for (i = descriptor_list; i; i = i->next)
      if (!i->connected)
        if (OUTSIDE(i->character)) write_to_q(messg, &i->output);
}
...
void send_to_except(char *messg, struct char_data *ch) {
  ...
  if (messg)
    for (i = descriptor_list; i; i = i->next)
      if (ch->desc != i && !i->connected) write_to_q(messg, &i->output);
}
...
void send_to_room_except(char *messg, int room, struct char_data *ch) {
  ...
  if (messg)
    for (i = world[room].people; i; i = i->next_in_room)
      if (i != ch && i->desc) write_to_q(messg, &i->desc->output);
}
...
void send_to_room_except_two(char *messg, int room, struct char_data *ch1,
  ...
  if (messg)
    for (i = world[room].people; i; i = i->next_in_room)
      if (i != ch1 && i != ch2 && i->desc) write_to_q(messg, &i->desc->output);
}
