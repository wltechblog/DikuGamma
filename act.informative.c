  ...
  for ((*j) = 0; (*j) < MAX_WEAR; (*j)++)
    if (equipment[(*j)])
      if (CAN_SEE_OBJ(ch, equipment[(*j)]))
        if (isname(arg, equipment[(*j)]->name)) return (equipment[(*j)]);
...
char *find_ex_description(char *word, struct extra_descr_data *list) {
  ...
  for (i = list; i; i = i->next)
    if (isname(word, i->keyword)) return (i->description);
...
void show_obj_to_char(struct obj_data *object, struct char_data *ch, int mode) {
  ...
  /*
          if (((mode == 2) || (mode == 4)) && (GET_ITEM_TYPE(object) ==
                  ...
                  if (mode == 2) list_obj_to_char(object->contains, ch,
     1,TRUE); if (mode == 4) list_obj_to_char(object->contains, ch, 3,TRUE);
          }
  */
}
...
void show_char_to_char(struct char_data *i, struct char_data *ch, int mode) {
  ...
  if (mode == 0) {
    ...
    if (!(i->player.long_descr) || (GET_POS(i) != i->specials.default_pos)) {
      /* A player char or a mobile without long descr, or not in default
       * pos. */
      if (!IS_NPC(i)) {
      ...
      if (IS_AFFECTED(i, AFF_INVISIBLE)) strcat(buffer, " (invisible)");
      ...
      if (IS_AFFECTED(ch, AFF_DETECT_EVIL)) {
        if (IS_EVIL(i)) strcat(buffer, " (Red Aura)");
      }
...
void do_look(struct char_data *ch, char *argument, int cmd) {
  ...
  char *tmp_desc;
  static char *keywords[] = {"north", "east", "south", "west", "up",
    ...
    switch (keyword_no) {
      ...
      case 7: {
        if (*arg2) {
          ...
          if (!found) {
            tmp_desc =
                find_ex_description(arg2, world[ch->in_room].ex_description);
            if (tmp_desc) {
            ...
            for (j = 0; j < MAX_WEAR && !found; j++) {
              if (ch->equipment[j]) {
                if (CAN_SEE_OBJ(ch, ch->equipment[j])) {
                  tmp_desc = find_ex_description(
                      arg2, ch->equipment[j]->ex_description);
                  if (tmp_desc) {
...
void do_weather(struct char_data *ch, char *argument, int cmd) {
  ...
  static char buf[100];
  char static *sky_look[4] = {"cloudless", "cloudy", "rainy",
...
void do_where(struct char_data *ch, char *argument, int cmd) {
        ...
        if (d->character && (d->connected == CON_PLYNG) &&
          ...
          if (d->original) /* If switched */
            sprintf(buf, "%-20s - %s [%d] In body of %s\n\r",
                    d->original->player.name, world[d->character->in_room].name,
  ...
  *buf = '\0';
  ...
  if (!*buf) send_to_char("Couldn't find any such thing.\n\r", ch);
}
...
void do_levels(struct char_data *ch, char *argument, int cmd) {
  ...
  *buf = '\0';
  ...
  for (i = 1; i < 21; i++) {
    sprintf(buf + strlen(buf), "%7d-%-7d : ", titles[GET_CLASS(ch) - 1][i].exp,
