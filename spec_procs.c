...
extern void add_follower(struct char_data *ch, struct char_data *victim);
...
int guild(struct char_data *ch, int cmd, char *arg) {
  ...
  static char *t_skills[] = {"sneak", /* No. 45 */
                             "hide",  "steal", "backstab", "pick", "\n"};
...
void exec_social(struct char_data *npc, char *cmd, int next_line, int *cur_line,
  ...
  if (GET_POS(npc) == POSITION_FIGHTING) return;
  ...
  switch (*cmd) {
    ...
    case '?': /* Test a random number */
      if (atoi(cmd + 1) <= number(1, 100)) ok = FALSE;
      break;
...
int snake(struct char_data *ch, int cmd, char *arg) {
  ...
  if (ch->specials.fighting &&
    ...
    cast_poison(GET_LEVEL(ch), ch, "", SPELL_TYPE_SPELL, ch->specials.fighting,
                0);
    return TRUE;
  }
  return FALSE;
...
int magic_user(struct char_data *ch, int cmd, char *arg) {
  ...
  for (vict = world[ch->in_room].people; vict; vict = vict->next_in_room)
    if (vict->specials.fighting == ch && number(0, 5) == 0) break;
...
int pet_shops(struct char_data *ch, int cmd, char *arg) {
  ...
  if (cmd == 59) { /* List */
    ...
    if (*pet_name) {
      ...
      sprintf(buf,
              "%sA small sign on a chain around the neck says 'My Name "
              "is %s'\n\r",
  ...
  switch (cmd) {
    case 10: /* get */
      if (!(chalice = get_obj_in_list_num(chl, world[ch->in_room].contents)) &&
        ...
        if (!(chalice =
                  get_obj_in_list_num(achl, world[ch->in_room].contents)) &&
            CAN_SEE_OBJ(ch, chalice))
          return (0);
      ...
      break;
    case 67: /* put */
      if (!(chalice = get_obj_in_list_num(chl, ch->carrying))) return (0);
