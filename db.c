  ...
  for (;;) {
    if (fgets(buf, 81, fl)) {
      if (*buf == '#') {
        ...
        else if (!(index = (struct index_data *)realloc(
                       index, (i + 1) * sizeof(struct index_data)))) {
          perror("load indices");
          exit(0);
        }
...
void allocate_room(int new_top) {
  ...
  if (new_top) {
    if (!(new_world = (struct room_data *)realloc(
              world, (new_top + 1) * sizeof(struct room_data)))) {
      perror("alloc_room");
      exit(0);
    }
  } else
...
void boot_zones(void) {
  ...
  for (;;) {
    ...
    else if (!(zone_table = (struct zone_data *)realloc(
                   zone_table, (zon + 1) * sizeof(struct zone_data)))) {
    ...
    for (expand = 1;;) {
      if (expand)
        ...
        else if (!(zone_table[zon].cmd = (struct reset_com *)realloc(
                       zone_table[zon].cmd,
      ...
      if (zone_table[zon].cmd[cmd_no].command == 'S') break;
...
void boot_zones(void) {
  ...
  for (;;) {
    ...
    for (expand = 1;;) {
      ...
      fscanf(fl, " %d %d %d", &zone_table[zon].cmd[cmd_no].if_flag,
...
struct char_data *read_mobile(int nr, int type) {
  ...
  if (letter == 'S') {
    ...
    for (i = 0; i < 3; i++) GET_COND(mob, i) = -1;
...
void reset_zone(int zone) {
  ...
  for (cmd_no = 0;; cmd_no++) {
    ...
    if (last_cmd || !ZCMD.if_flag) switch (ZCMD.command) {
...
void store_to_char(struct char_file_u *st, struct char_data *ch) {
  ...
  for (i = 0; i <= MAX_TOUNGE - 1; i++) ch->player.talks[i] = st->talks[i];
  ...
  for (i = 0; i <= MAX_SKILLS - 1; i++) ch->skills[i] = st->skills[i];
  ...
  for (i = 0; i < MAX_AFFECT; i++) {
    if (st->affected[i].type) affect_to_char(ch, &st->affected[i]);
  }
...
void char_to_store(struct char_data *ch, struct char_file_u *st) {
  ...
  for (i = 0; i <= MAX_TOUNGE - 1; i++) st->talks[i] = ch->player.talks[i];
...
void save_char(struct char_data *ch, sh_int load_room) {
  ...
  if (!expand) fseek(fl, ch->desc->pos * sizeof(struct char_file_u), 0);
...
void init_char(struct char_data *ch) {
  ...
  for (i = 0; i < MAX_TOUNGE; i++) ch->player.talks[i] = 0;
  ...
  for (i = 0; i < 5; i++) ch->specials.apply_saving_throw[i] = 0;

  for (i = 0; i < 3; i++) GET_COND(ch, i) = (GET_LEVEL(ch) == 24 ? -1 : 24);
}
...
int real_room(int virtual) {
  ...
  for (;;) {
    ...
    if ((world + mid)->number == virtual) return (mid);
    if (bot >= top) {
...
int real_mobile(int virtual) {
  ...
  for (;;) {
    ...
    if ((mob_index + mid)->virtual == virtual) return (mid);
    if (bot >= top) return (-1);
...
int real_object(int virtual) {
  ...
  for (;;) {
    ...
    if ((obj_index + mid)->virtual == virtual) return (mid);
    if (bot >= top) return (-1);
