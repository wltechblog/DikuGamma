...
/* extern functions */
extern void damage(struct char_data *ch, struct char_data *victim, int damage,
                   int weapontype);
    ...
    if (victim) {
      if (victim == ch) {
        ...
        if (IS_AFFECTED(ch, AFF_CHARM) && (ch->master == victim)) {
          act("$N is just such a good friend, you simply can't hit "
              "$M.",
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (CAN_GO(ch, attempt) &&
      ...
      if ((die = do_simple_move(ch, attempt, FALSE)) == 1) {
        /* The escape has succeded */
        loose =
            GET_MAX_HIT(ch->specials.fighting) - GET_HIT(ch->specials.fighting);
        loose *= GET_LEVEL(ch->specials.fighting);

        if (!IS_NPC(ch)) gain_exp(ch, -loose);
...
void do_rescue(struct char_data *ch, char *argument, int cmd) {
  ...
  if (GET_CLASS(ch) != CLASS_WARRIOR)
    ...
    if (victim->specials.fighting == tmp_ch) stop_fighting(victim);
    if (tmp_ch->specials.fighting) stop_fighting(tmp_ch);
