...
void do_at(struct char_data *ch, char *argument, int cmd) {
  ...
  for (target_mob = world[location].people; target_mob;
...
void do_stat(struct char_data *ch, char *argument, int cmd) {
  ...
  if (!*arg1) {
    ...
    if (j = get_obj_vis(ch, arg1)) {
      ...
      switch (j->obj_flags.type_flag) {
        ...
        case ITEM_POTION:
          ...
          break;
        case ITEM_TRAP:
          sprintf(buf, "Spell : %d\n\r- Hitpoints : %d", j->obj_flags.value[0],
                  j->obj_flags.value[1]);
          break;
        case ITEM_CONTAINER:
      ...
      if (!found) strcpy(buf, "Contains : Nothing\n\r");
      send_to_char(buf, ch);
    ...
    if (k = get_char_vis(ch, arg1)) {
      ...
      sprintf(buf,
              "AC:[%d/10], Coins: [%d], Exp: [%d], Hitroll: [%d], Damroll: "
              "[%d]\n\r",
      ...
      sprintf(
          buf, "Position: %s, Fighting: %s", buf2,
...
void do_force(struct char_data *ch, char *argument, int cmd) {
  ...
  char name[100], to_force[100], buf[250];
...
void roll_abilities(struct char_data *ch) {
  ...
  for (i = 0; i < 5; i++) {
    for (j = 0; j < 4; j++) rools[j] = number(1, 6);
  ...
  switch (GET_CLASS(ch)) {
    ...
    case CLASS_WARRIOR: {
      ...
      if (ch->abilities.str == 18) ch->abilities.str_add = number(0, 100);
    } break;
  }
...
void do_advance(struct char_data *ch, char *argument, int cmd) {
  ...
  if (GET_LEVEL(victim) == 0) {
    ...
    if (GET_LEVEL(victim) < 24) {
      gain_exp_regardless(
          victim, (titles[GET_CLASS(victim) - 1][GET_LEVEL(victim) + adv].exp) -
                      GET_EXP(victim));
