...
bool recep_offer(struct char_data *ch, struct char_data *receptionist,
  ...
  if (cost->total_cost > GET_GOLD(ch)) {
    if (GET_LEVEL(ch) < 21)
      act("$n tells you 'Which I can see you can't afford'", FALSE,
          receptionist, 0, ch, TO_VICT);
    else {
      act("$n tells you 'Well, since you're a God, I guess it's okay'", FALSE,
          receptionist, 0, ch, TO_VICT);
      cost->total_cost = 0;
    }
  }
...
void load_char_objs(struct char_data *ch) {
  ...
  if (found) {
    ...
    timegold = (int)((st.total_cost * (time(0) - st.last_update)) /
                     (SECS_PER_REAL_DAY));
...
void obj_to_store(struct obj_data *obj, struct obj_file_u *st,
  ...
  if (obj) {
    ...
    if ((obj->obj_flags.timer < 0) && (obj->obj_flags.timer != OBJ_NOTIMER)) {
      sprintf(buf,
              "You're told: 'The %s is just old junk, I'll throw it away for "
              "you.'\n\r",
              fname(obj->name));
      send_to_char(buf, ch);
    } else {
...
void update_obj_file(void) {
  ...
  if (!(char_file = fopen(PLAYER_FILE, "r+"))) {
    perror(
        "   Opening player file for reading. (reception.c, "
        "update_obj_file)");
    exit(1);
  }
    ...
    if ((!feof(fl)) && (no_read > 0) && st.owner[0]) {
      ...
      if (days_passed > 0) {
        if ((st.total_cost * days_passed) > st.gold_left) {
          ...
          fseek(char_file,
                (long)(player_table[player_i].nr * sizeof(struct char_file_u)),
                0);
...
int receptionist(struct char_data *ch, int cmd, char *arg) {
  ...
  for (temp_char = world[ch->in_room].people; (temp_char) && (!recep);
    ...
    if (IS_MOB(temp_char))
      if (mob_index[temp_char->nr].func == receptionist) recep = temp_char;
  ...
  if ((cmd != 92) && (cmd != 93)) {
    if (!number(0, 30)) do_action(recep, "", action_tabel[number(0, 8)]);
    return (FALSE);
  }
