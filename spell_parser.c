...
#define USE_MANA(ch, sn)           \
  MAX(spell_info[sn].min_usesmana, \
      100 / (2 + GET_LEVEL(ch) - SPELL_LEVEL(ch, sn)))
...
char *spells[] = {"armor", /* 1 */
                  "teleport", "bless", "blindness", "burning hands",
                  "call lightning", "charm person", "chill touch", "clone",
                  "colour spray", "control weather", /* 11 */
                  "create food", "create water", "cure blind", "cure critic",
                  "cure light", "curse", "detect evil", "detect invisibility",
                  "detect magic", "detect poison", /* 21 */
                  "dispel evil", "earthquake", "enchant weapon", "energy drain",
                  "fireball", "harm", "heal", "invisibility", "lightning bolt",
                  "locate object", /* 31 */
                  "magic missile", "poison", "protection from evil",
                  "remove curse", "sanctuary", "shocking grasp", "sleep",
                  "strength", "summon", "ventriloquate",           /* 41 */
                  "word of recall", "remove poison", "sense life", /* 44 */

                  /* RESERVED SKILLS */
                  "SKILL_SNEAK", /* 45 */
                  "SKILL_HIDE", "SKILL_STEAL", "SKILL_BACKSTAB",
                  "SKILL_PICK_LOCK", "SKILL_KICK", /* 50 */
                  "SKILL_BASH", "SKILL_RESCUE",
...
const byte saving_throws[4][5][25] = {
    {{16, 14, 14, 14, 14, 14, 13, 13, 13, 13, 13, 11, 11,
      11, 11, 11, 10, 10, 10, 10, 10, 8,  6,  4,  0},
     {13, 11, 11, 11, 11, 11, 9, 9, 9, 9, 9, 7, 7,
      7,  7,  7,  5,  5,  5,  5, 5, 3, 2, 1, 0},
     {15, 13, 13, 13, 13, 13, 11, 11, 11, 11, 11, 9, 9,
      9,  9,  9,  7,  7,  7,  7,  7,  5,  4,  2,  0},
     {17, 15, 15, 15, 15, 15, 13, 13, 13, 13, 13, 11, 11,
      11, 11, 11, 9,  9,  9,  9,  9,  7,  5,  3,  0},
     {14, 12, 12, 12, 12, 12, 10, 10, 10, 10, 10, 8, 8,
      8,  8,  8,  6,  6,  6,  6,  6,  4,  3,  2,  0}},
    {{11, 10, 10, 10, 9, 9, 9, 7, 7, 7, 6, 6, 6,
      5,  5,  5,  4,  4, 4, 2, 2, 2, 2, 1, 0},
     {16, 14, 14, 14, 13, 13, 13, 11, 11, 11, 10, 10, 10,
      9,  9,  9,  8,  8,  8,  6,  6,  5,  4,  3,  0},
     {15, 13, 13, 13, 12, 12, 12, 10, 10, 10, 9, 9, 9,
      8,  8,  8,  7,  7,  7,  5,  5,  4,  3,  2, 0},
     {18, 16, 16, 16, 15, 15, 15, 13, 13, 13, 12, 12, 12,
      11, 11, 11, 10, 10, 10, 8,  8,  7,  6,  5,  0},
     {17, 15, 15, 15, 14, 14, 14, 12, 12, 12, 11, 11, 11,
      10, 10, 10, 9,  9,  9,  7,  7,  6,  5,  4,  0}},
    {{15, 13, 13, 13, 13, 12, 12, 12, 12, 11, 11, 11, 11,
      10, 10, 10, 10, 9,  9,  9,  9,  8,  7,  6,  0},
     {16, 14, 14, 14, 14, 12, 12, 12, 12, 10, 10, 10, 10,
      8,  8,  8,  8,  6,  6,  6,  6,  4,  3,  2,  0},
     {14, 12, 12, 12, 12, 11, 11, 11, 11, 10, 10, 10, 10,
      9,  9,  9,  9,  8,  8,  8,  8,  7,  5,  3,  0},
     {18, 16, 16, 16, 16, 15, 15, 15, 15, 14, 14, 14, 14,
      13, 13, 13, 13, 12, 12, 12, 12, 11, 9,  5,  0},
     {17, 15, 15, 15, 15, 13, 13, 13, 13, 11, 11, 11, 11,
      9,  9,  9,  9,  7,  7,  7,  7,  5,  3,  1,  0}},
    {{16, 14, 14, 13, 13, 11, 11, 10, 10, 8, 8, 7, 7,
      5,  5,  4,  4,  3,  3,  3,  3,  2,  2, 1, 0},
     {18, 16, 16, 15, 15, 13, 13, 12, 12, 10, 10, 9, 9,
      7,  7,  6,  6,  5,  5,  5,  5,  4,  3,  2,  0},
     {17, 15, 15, 14, 14, 12, 12, 11, 11, 9, 9, 8, 8,
      6,  6,  5,  5,  4,  4,  4,  4,  3,  2, 1, 0},
     {20, 17, 17, 16, 16, 13, 13, 12, 12, 9, 9, 8, 8,
      5,  5,  4,  4,  4,  4,  4,  4,  3,  2, 1, 0},
     {19, 17, 17, 16, 16, 14, 14, 13, 13, 11, 11, 10, 10,
      8,  8,  7,  7,  6,  6,  6,  6,  4,  2,  1,  0}}};
...
void clone_char(struct char_data *ch) {
  ...
  for (af = ch->affected; af; af = af->next) affect_to_char(clone, af);

  for (i = 0; i < 3; i++) GET_COND(clone, i) = GET_COND(ch, i);

  clone->points = ch->points;

  for (i = 0; i < MAX_SKILLS; i++) clone->skills[i] = ch->skills[i];
...
void say_spell(struct char_data *ch, int si) {
  char buf[MAX_STRING_LENGTH - 128], splwd[MAX_BUF_LENGTH];
...
bool saves_spell(struct char_data *ch, sh_int save_type) {
  ...
  if (!IS_NPC(ch)) {
    ...
    if (GET_LEVEL(ch) > 20) return (TRUE);
  }
      ...
      if (!IS_SET(spell_info[spl].targets, TAR_IGNORE)) {
        ...
        if (*name) {
          if (IS_SET(spell_info[spl].targets, TAR_CHAR_ROOM))
            if (tar_char = get_char_room_vis(ch, name)) target_ok = TRUE;

          if (!target_ok && IS_SET(spell_info[spl].targets, TAR_CHAR_WORLD))
            if (tar_char = get_char_vis(ch, name)) target_ok = TRUE;
          ...
          if (!target_ok && IS_SET(spell_info[spl].targets, TAR_OBJ_WORLD))
            if (tar_obj = get_obj_vis(ch, name)) target_ok = TRUE;
...
void assign_spell_pointers(void) {
  ...
  for (i = 0; i < MAX_SPL_LIST; i++) spell_info[i].spell_pointer = 0;

  /* From spells1.c */

  SPELLO(32, 12, POSITION_FIGHTING, 1, 21, 15, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
         cast_magic_missile);

  SPELLO(8, 12, POSITION_FIGHTING, 3, 21, 15, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
         cast_chill_touch);

  SPELLO(5, 12, POSITION_FIGHTING, 5, 21, 15, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
         cast_burning_hands);

  SPELLO(37, 12, POSITION_FIGHTING, 7, 21, 15, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
         cast_shocking_grasp);

  SPELLO(30, 12, POSITION_FIGHTING, 9, 21, 15, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
         cast_lightning_bolt);

  SPELLO(10, 12, POSITION_FIGHTING, 11, 21, 15, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
         cast_colour_spray);

  SPELLO(25, 12, POSITION_FIGHTING, 13, 21, 35, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
         cast_energy_drain);

  SPELLO(26, 12, POSITION_FIGHTING, 15, 21, 15, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
         cast_fireball);

  SPELLO(23, 12, POSITION_FIGHTING, 21, 7, 15, TAR_IGNORE, cast_earthquake);

  SPELLO(22, 12, POSITION_FIGHTING, 21, 10, 15, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
         cast_dispel_evil);

  SPELLO(6, 12, POSITION_FIGHTING, 21, 12, 15, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
         cast_call_lightning);

  SPELLO(27, 12, POSITION_FIGHTING, 21, 15, 35, TAR_CHAR_ROOM | TAR_FIGHT_VICT,
         cast_harm);
  ...
  SPELLO(7, 12, POSITION_STANDING, 14, 21, 5, TAR_CHAR_ROOM | TAR_SELF_NONO,
         cast_charm_person);

  SPELLO(9, 12, POSITION_STANDING, 15, 21, 40, TAR_CHAR_ROOM, cast_clone);

  SPELLO(11, 12, POSITION_STANDING, 10, 13, 25, TAR_IGNORE,
         cast_control_weather);

  SPELLO(12, 12, POSITION_STANDING, 21, 3, 5, TAR_IGNORE, cast_create_food);

  SPELLO(13, 12, POSITION_STANDING, 21, 2, 5, TAR_OBJ_INV | TAR_OBJ_EQUIP,
         cast_create_water);

  SPELLO(14, 12, POSITION_STANDING, 21, 4, 5, TAR_CHAR_ROOM, cast_cure_blind);

  SPELLO(15, 12, POSITION_FIGHTING, 21, 9, 20, TAR_CHAR_ROOM, cast_cure_critic);

  SPELLO(16, 12, POSITION_FIGHTING, 21, 1, 15, TAR_CHAR_ROOM, cast_cure_light);

  SPELLO(17, 12, POSITION_STANDING, 12, 21, 20,
         TAR_CHAR_ROOM | TAR_OBJ_ROOM | TAR_OBJ_INV | TAR_OBJ_EQUIP,
         cast_curse);

  SPELLO(18, 12, POSITION_STANDING, 21, 4, 5, TAR_CHAR_ROOM | TAR_SELF_ONLY,
         cast_detect_evil);

  SPELLO(19, 12, POSITION_STANDING, 2, 5, 5, TAR_CHAR_ROOM | TAR_SELF_ONLY,
         cast_detect_invisibility);

  SPELLO(20, 12, POSITION_STANDING, 2, 3, 5, TAR_CHAR_ROOM | TAR_SELF_ONLY,
         cast_detect_magic);

  SPELLO(21, 12, POSITION_STANDING, 21, 2, 5,
         TAR_CHAR_ROOM | TAR_OBJ_INV | TAR_OBJ_EQUIP, cast_detect_poison);

  SPELLO(24, 12, POSITION_STANDING, 12, 21, 100, TAR_OBJ_INV | TAR_OBJ_EQUIP,
         cast_enchant_weapon);
  ...
  SPELLO(31, 12, POSITION_STANDING, 6, 21, 20, TAR_OBJ_WORLD,
         cast_locate_object);
  ...
  SPELLO(34, 12, POSITION_STANDING, 21, 6, 5, TAR_CHAR_ROOM | TAR_SELF_ONLY,
  ...
  SPELLO(36, 12, POSITION_STANDING, 21, 13, 75, TAR_CHAR_ROOM, cast_sanctuary);

  SPELLO(38, 12, POSITION_STANDING, 14, 21, 15, TAR_CHAR_ROOM, cast_sleep);

  SPELLO(39, 12, POSITION_STANDING, 7, 21, 20, TAR_CHAR_ROOM | TAR_SELF_ONLY,
         cast_strength);
  ...
  SPELLO(42, 12, POSITION_STANDING, 21, 11, 5, TAR_CHAR_ROOM | TAR_SELF_ONLY,
         cast_word_of_recall);

  SPELLO(43, 12, POSITION_STANDING, 21, 9, 5,
         TAR_CHAR_ROOM | TAR_OBJ_INV | TAR_OBJ_ROOM, cast_remove_poison);

  SPELLO(44, 12, POSITION_STANDING, 21, 7, 5, TAR_CHAR_ROOM | TAR_SELF_ONLY,
         cast_sense_life);
