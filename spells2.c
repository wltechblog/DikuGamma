...
void cast_armor(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_POTION:
      if (affected_by_spell(ch, SPELL_ARMOR)) return;
      spell_armor(level, ch, ch, 0);
      break;
    case SPELL_TYPE_SCROLL:
      if (tar_obj) return;
      if (!tar_ch) tar_ch = ch;
      if (affected_by_spell(tar_ch, SPELL_ARMOR)) return;
      spell_armor(level, ch, ch, 0);
      break;
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
        ...
        if (tar_ch != ch) spell_teleport(level, ch, tar_ch, 0);
      break;
...
void cast_bless(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_SCROLL:
      if (tar_obj) { /* It's an object */
        if (IS_SET(tar_obj->obj_flags.extra_flags, ITEM_BLESS)) return;
        spell_bless(level, ch, 0, tar_obj);
      } else { /* Then it is a PC | NPC */
...
void cast_clone(byte level, struct char_data *ch, char *arg, int type,
  ...
  /*
    switch (type) {
      case SPELL_TYPE_SPELL:
                          if (tar_ch) {
                                  sprintf(buf, "You create a duplicate of
    %s.\n\r", GET_NAME(tar_ch)); send_to_char(buf, ch); sprintf(buf, "%%s
    creates a duplicate of %s,\n\r", GET_NAME(tar_ch)); perform(buf, ch,
    FALSE);
    ...
    %s,\n\r",SANA(tar_obj),tar_obj->short_description); perform(buf, ch,
    FALSE);
...
void cast_cure_blind(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_cure_blind(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_cure_critic(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_cure_critic(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_cure_light(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_cure_light(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_curse(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_curse(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_detect_evil(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    case SPELL_TYPE_SPELL:
      ...
      break;
    case SPELL_TYPE_POTION:
      if (affected_by_spell(ch, SPELL_DETECT_EVIL)) return;
...
void cast_detect_invisibility(byte level, struct char_data *ch, char *arg,
  ...
  switch (type) {
    case SPELL_TYPE_SPELL:
      ...
      break;
    case SPELL_TYPE_POTION:
      if (affected_by_spell(ch, SPELL_DETECT_INVISIBLE)) return;
...
void cast_detect_magic(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    case SPELL_TYPE_SPELL:
      ...
      break;
    case SPELL_TYPE_POTION:
      if (affected_by_spell(ch, SPELL_DETECT_MAGIC)) return;
...
void cast_dispel_evil(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_dispel_evil(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_heal(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_heal(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_invisibility(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    case SPELL_TYPE_SPELL:
      ...
      break;
    case SPELL_TYPE_POTION:
      if (!IS_AFFECTED(ch, AFF_INVISIBLE)) spell_invisibility(level, ch, ch, 0);
      break;
    case SPELL_TYPE_SCROLL:
      if (tar_obj) {
        if (!(IS_SET(tar_obj->obj_flags.extra_flags, ITEM_INVISIBLE)))
...
void cast_poison(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_poison(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_protection_from_evil(byte level, struct char_data *ch, char *arg,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_protection_from_evil(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_remove_curse(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_remove_curse(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_remove_poison(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_remove_poison(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_sanctuary(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_sanctuary(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_sleep(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_sleep(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_strength(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_strength(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_charm_person(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_charm_person(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_sense_life(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    ...
    case SPELL_TYPE_STAFF:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_sense_life(level, ch, tar_ch, 0);
      break;
    default:
...
void cast_gas_breath(byte level, struct char_data *ch, char *arg, int type,
  ...
  switch (type) {
    case SPELL_TYPE_SPELL:
      for (tar_ch = world[ch->in_room].people; tar_ch;
           tar_ch = tar_ch->next_in_room)
        if (tar_ch != ch) spell_gas_breath(level, ch, tar_ch, 0);
      break;
      /* THIS ONE HURTS!! */
    default:
