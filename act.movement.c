...
*/
{
  ...
  if (GET_LEVEL(ch) < 21 && !IS_NPC(ch)) GET_MOVE(ch) -= need_movement;
...
int find_door(struct char_data *ch, char *type, char *dir) {
  ...
  } else /* try to locate the keyword */
  {
    for (door = 0; door <= 5; door++)
      if (EXIT(ch, door))
        if (EXIT(ch, door)->keyword)
          if (isname(type, EXIT(ch, door)->keyword)) return (door);
...
int has_key(struct char_data *ch, int key) {
  ...
  for (o = ch->carrying; o; o = o->next_content)
    if (obj_index[o->item_number].virtual == key) return (1);

  if (ch->equipment[HOLD])
    if (obj_index[ch->equipment[HOLD]->item_number].virtual == key) return (1);
...
void do_lock(struct char_data *ch, char *argument, int cmd) {
  ...
  if (!*type)
    ...
    if (!IS_SET(EXIT(ch, door)->exit_info, EX_ISDOOR))
      ...
      if ((other_room = EXIT(ch, door)->to_room) != NOWHERE)
        if (back = world[other_room].dir_option[rev_dir[door]])
          if (back->to_room == ch->in_room) SET_BIT(back->exit_info, EX_LOCKED);
    }
}
