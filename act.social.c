...
void boot_social_messages(void) {
  ...
  for (;;) {
    ...
    if (!soc_mess_list) {
      ...
      list_top = 0;
    } else if (!(soc_mess_list = (struct social_messg *)realloc(
                     soc_mess_list,
    ...
    if (!soc_mess_list[list_top].char_found) continue;
...
int find_action(int cmd) {
  ...
  for (;;) {
    ...
    if (soc_mess_list[mid].act_nr == cmd) return (mid);
    if (bot == top) return (-1);
          ...
          case 0: {
            if (GET_SEX(ch) == SEX_MALE) {
              ...
              if (GET_SEX(victim) == SEX_MALE)
                ...
                act("$n tells you that you'd loose a beautycontest "
                    "against a "
                    "troll.",
                    FALSE, ch, 0, victim, TO_VICT);
            }
...
void boot_pose_messages(void) {
  ...
  for (counter = 0;; counter++) {
    ...
    if (pose_messages[counter].level < 0) break;
