...
#define READ_TITLE(ch)                                                        \
  (GET_SEX(ch) == SEX_MALE ? titles[GET_CLASS(ch) - 1][GET_LEVEL(ch)].title_m \
...
int move_limit(struct char_data *ch) {
  ...
  if (!IS_NPC(ch)) /* HERE SHOULD BE CON CALCULATIONS INSTEAD */
...
void advance_level(struct char_data *ch) {
  ...
  if (GET_LEVEL(ch) > 20)
    for (i = 0; i < 3; i++) ch->specials.conditions[i] = -1;
}
...
void gain_exp_regardless(struct char_data *ch, int gain) {
  ...
  if (!IS_NPC(ch)) {
    if (gain > 0) {
      GET_EXP(ch) += gain;
      for (i = 0; (i < 25) && (titles[GET_CLASS(ch) - 1][i].exp <= GET_EXP(ch));
           i++) {
        if (i > GET_LEVEL(ch)) {
          send_to_char("You raise a level\n\r", ch);
          GET_LEVEL(ch) = i;
...
void gain_condition(struct char_data *ch, int condition, int value) {
  ...
  switch (condition) {
    ...
    case DRUNK: {
      if (intoxicated) send_to_char("You are now sober.\n\r", ch);
      return;
    }
    default:
      break;
...
void point_update(void) {
  ...
  for (i = character_list; i; i = next_dude) {
    ...
    if (!IS_NPC(i)) {
      ...
      if (GET_LEVEL(i) < 22) check_idling(i);
    }
    ...
    if ((GET_ITEM_TYPE(j) == ITEM_CONTAINER) && (j->obj_flags.value[3])) {
      ...
      if (!j->obj_flags.timer) {
        if (j->carried_by)
          ...
          act("A quivering hoard of maggots consume $p.", TRUE,
