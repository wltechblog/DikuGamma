...
void affect_remove(struct char_data *ch, struct affected_type *af) {
  ...
  if (ch->affected == af) {
    ...
    if (hjp->next != af) {
      dm_log(
          "FATAL : Could not locate affected_type in ch->affected. "
          "(handler.c, "
          "affect_remove)");
      exit(1);
    }
...
void affect_from_char(struct char_data *ch, byte skill) {
  ...
  for (hjp = ch->affected; hjp; hjp = hjp->next)
    if (hjp->type == skill) affect_remove(ch, hjp);
}
...
void char_from_room(struct char_data *ch) {
  ...
  {
    for (i = world[ch->in_room].people; i->next_in_room != ch;
         i = i->next_in_room)
      ;
...
int apply_ac(struct char_data *ch, int eq_pos) {
  ...
  if (!(GET_ITEM_TYPE(ch->equipment[eq_pos]) == ITEM_ARMOR)) return 0;
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos) {
  ...
  if (GET_ITEM_TYPE(obj) == ITEM_ARMOR) GET_AC(ch) -= apply_ac(ch, pos);
...
void obj_from_room(struct obj_data *object) {
  ...
  {
    for (i = world[object->in_room].contents; i && (i->next_content != object);
         i = i->next_content)
      ;
...
void obj_from_obj(struct obj_data *obj) {
  ...
  if (obj->in_obj) {
    ...
    if (tmp->carried_by) IS_CARRYING_W(tmp->carried_by) -= GET_OBJ_WEIGHT(obj);
...
void extract_obj(struct obj_data *obj) {
  ...
  if (object_list == obj) /* head of list */
    ...
    if (temp1) temp1->next = obj->next;
  }

  if (obj->item_number >= 0) (obj_index[obj->item_number].number)--;
  free_obj(obj);
}
...
void update_char_objects(struct char_data *ch) {
  ...
  for (i = 0; i < MAX_WEAR; i++)
    if (ch->equipment[i]) update_object(ch->equipment[i], 2);
...
void extract_char(struct char_data *ch) {
  ...
  if (!IS_NPC(ch) && !ch->desc) {
    for (t_desc = descriptor_list; t_desc; t_desc = t_desc->next)
      if (t_desc->original == ch) do_return(t_desc->character, "", 0);
  }
  ...
  for (k = combat_list; k; k = next_char) {
    ...
    if (k->specials.fighting == ch) stop_fighting(k);
  }
  ...
  for (l = 0; l < MAX_WEAR; l++)
    if (ch->equipment[l]) obj_to_room(unequip_char(ch, l), was_in);
  ...
  if (ch == character_list)
    ...
    if (k)
      ...
      dm_log(
          "Trying to remove ?? from character_list. (handler.c, "
          "extract_char)");
      abort();
    }
  }
  ...
  if (ch->desc) {
    if (ch->desc->original) do_return(ch, "", 0);
...
struct char_data *get_char_vis(struct char_data *ch, char *name) {
  ...
  /* check location */
  if (i = get_char_room_vis(ch, name)) return (i);
...
struct obj_data *get_obj_vis(struct char_data *ch, char *name) {
  ...
  /* scan items carried */
  if (i = get_obj_in_list_vis(ch, name, ch->carrying)) return (i);
...
int generic_find(char *arg, int bitvector, struct char_data *ch,
  ...
  while (*arg && !found) {
    ...
    name[i] = 0;
    arg += i;
    if (search_block(name, ignore, TRUE) > -1) found = TRUE;
  }
