...
char *string_fields[] = {"name",        "short", "long",
                         "description", "title", "delete-description",
                         "\n"};
      ...
      case 6:
        ...
        for (ed = obj->ex_description;; ed = ed->next)
          if (!ed) {
            ...
            free(ed->keyword);
            if (ed->description) free(ed->description);
...
char *one_word(char *argument, char *first_arg) {
  ...
  do {
    ...
    if (*(argument + begin) == '\"') { /* is it a quote */
      ...
      if (*(argument + begin + look_at) == '\"') begin++;
    } else {
...
void check_reboot(void) {
  ...
  extern int shutdown, autoreboot;
 ...
 * right - but I couldn't think of anything better), change it, or don't use
 * -l. It shouldn't be necessary to use -l anyhow. It's oppressive and
 * unchristian to harness man's desire to play. Who needs a friggin' degree,
 * anyhow?
 */
...
int load(void) {
  ...
  if (p_point < 0) {
    ...
    for (i = 1; i < 5; i++) previous[i] = previous[0];
    p_point = 1;
    return (previous[0]);
  } else {
    /* put new figure in table */
    previous[p_point] = atoi(info.sl_load1);
    if (++p_point > 4) p_point = 0;

    for (i = 0, sum = 0; i < 5; i++) sum += previous[i];
    return ((int)sum / 5);
  }
}
